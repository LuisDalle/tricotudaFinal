...
CREATE TABLE `tb_imagens` (
  `PK_ImagemID` int NOT NULL AUTO_INCREMENT,
  `FK_ProdutoID` int NOT NULL,
  `TX_CaminhoImagem` varchar(255) NOT NULL,
  `DT_Upload` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`PK_ImagemID`),
  KEY `FK_ProdutoID` (`FK_ProdutoID`),
  CONSTRAINT `tb_imagens_ibfk_1` FOREIGN KEY (`FK_ProdutoID`) REFERENCES `tb_produtos` (`PK_ProdutoID`)
