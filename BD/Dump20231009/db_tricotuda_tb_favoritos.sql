...
CREATE TABLE `tb_favoritos` (
  `PK_FavoritosID` int NOT NULL AUTO_INCREMENT,
  `FK_ClienteID` int NOT NULL,
  `FK_ProdutoID` int NOT NULL,
  `DT_Adicionado` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`PK_FavoritosID`),
  KEY `FK_ClienteID` (`FK_ClienteID`),
  KEY `FK_ProdutoID` (`FK_ProdutoID`),
  CONSTRAINT `tb_favoritos_ibfk_1` FOREIGN KEY (`FK_ClienteID`) REFERENCES `tb_clientes` (`PK_ClienteID`),
  CONSTRAINT `tb_favoritos_ibfk_2` FOREIGN KEY (`FK_ProdutoID`) REFERENCES `tb_produtos` (`PK_ProdutoID`)
