...
CREATE TABLE `tb_avaliacoes` (
  `PK_AvaliacaoID` int NOT NULL AUTO_INCREMENT,
  `FK_ClienteID` int NOT NULL,
  `FK_ProdutoID` int NOT NULL,
  `NM_Classificacao` int NOT NULL,
  `TX_Comentario` text,
  ...
  PRIMARY KEY (`PK_AvaliacaoID`),
  KEY `FK_ClienteID` (`FK_ClienteID`),
  KEY `FK_ProdutoID` (`FK_ProdutoID`),
  CONSTRAINT `tb_avaliacoes_ibfk_1` FOREIGN KEY (`FK_ClienteID`) REFERENCES `tb_clientes` (`PK_ClienteID`),
  CONSTRAINT `tb_avaliacoes_ibfk_2` FOREIGN KEY (`FK_ProdutoID`) REFERENCES `tb_produtos` (`PK_ProdutoID`)
